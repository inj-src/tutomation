...
function LoaderGrid({
  ...
  return (
    <span aria-hidden className="gap-[1.5px] grid grid-cols-[repeat(3,4px)] shrink-0">
...
export default function LoadingState({
  ...
  const elapsedEl = (
    <span className="font-mono tabular-nums text-[12px] text-ink-3">{elapsed}</span>
  );

  if (surfer) {
    return (
      <div role="status" className="flex flex-col items-start w-fit">
        ...
        <div
          className="shadow-overlay mt-2 rounded-[10px] w-56 overflow-hidden"
          style={{
            animation: "pop-in 200ms cubic-bezier(0.16,1,0.3,1) both",
            transformOrigin: "top left",
          }}
        >
          <div className="relative w-full aspect-video" style={{ background: "var(--tooltip-bg)" }}>
            {videoOk ? (
              <video
                src={videoSrc}
                autoPlay
              ...
              <div className="flex flex-col justify-center items-center gap-1.5 w-full h-full">
                <LoaderGrid {...PATTERNS.Drive} />
                <span
                  className="px-3 font-mono text-[10px] text-center"
  ...
  return (
    <div role="status" className="flex items-center gap-2.5 w-fit">
