...
const EXCALIDRAW_ENTRY = fileURLToPath(
  new URL("./excalidraw-browser.ts", import.meta.url)
)
const EXCALIFONT_DIRECTORY = fileURLToPath(
  new URL(
...
async function readExcalifont(): Promise<string> {
  ...
  if (!fontName) {
    throw new Error(
      "Could not find Excalifont in the installed Excalidraw package."
    )
  }

  return (await readFile(`${EXCALIFONT_DIRECTORY}/${fontName}`)).toString(
    "base64"
  )
}
...
function enlargedTick(
  ...
  const tickScale = Math.min(
    2.5,
    Math.max(1, (32 * scale) / currentWidth, (24 * scale) / currentHeight)
  )
  const scaledPoints = absolutePoints.map(
    ([x, y]) =>
      [
    ...
    case "oval": {
      ...
      if (!center || annotation.radius === null) {
        if (
          ...
          annotation.radiusX === null ||
          annotation.radiusY === null
        ) {
          break
        }
      }
      ...
      const x = Math.min(
        width - diameterX,
        Math.max(0, center[0] - diameterX / 2)
      )
      const y = Math.min(
        height - diameterY,
        Math.max(0, center[1] - diameterY / 2)
      )
      elements.push({
        type: "ellipse",
        x,
...
function annotationComment(
  ...
  if (
    ...
    (annotation.radius !== null ||
      (annotation.radiusX !== null && annotation.radiusY !== null))
  ) {
    const center = point(annotation.center, width, height)
    if (!center) {
      return null
...
function buildElements(
  ...
  const scores: RenderElement[] = evaluation.questionScores.flatMap(
    (questionScore) => {
      ...
      const scoreY = Math.max(
        0,
        Math.min(
          height - SCORE_FONT_SIZE * scale,
          anchorY - (SCORE_FONT_SIZE * scale) / 2
        )
      )
  ...
  return [
    ...scores,
    ...evaluation.annotations.flatMap((annotation) =>
      annotationElements(annotation, width, height, scale)
    ),
  ]
}
...
function scaleEvaluation(
  ...
  return {
    ...
    annotations: evaluation.annotations.map((annotation) => ({
      ...
      points:
        annotation.points?.map((value) => scalePoint(value, scale) ?? []) ??
        null,
    })),
  }
}
  ...
  try {
    ...
    return await page.evaluate(
      async (input) => {
        ...
        const render = (
          window as Window & {
            __tutomationRenderExcalidraw?: (value: {
              width: number
              height: number
