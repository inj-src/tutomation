...
function CategoryWorkspace() {
  ...
  const { entries, entryList, selected, selectedId } = useCategoryEntries(examId, entryParam);
  ...
  const capture = useQuery({
    ...
    retry: false,
    refetchInterval: (query) => (query.state.data?.status === "capturing" ? 500 : false),
  });
  ...
  const questionUrl =
    capture.data?.evaluationUrl ?? (selected ? candidateEvaluationUrl(selected) : undefined);
  ...
  const busy = entries.isLoading || capture.isLoading || capture.data?.status !== "ready";
          ...
          <SidebarInset className="min-w-0 min-h-0">
            <div className="flex flex-1 min-h-0">
              <ResizablePanelGroup orientation="horizontal" className="flex-1 min-w-0 min-h-0">
                <ResizablePanel defaultSize="74%" minSize="42%" className="min-w-0 h-dvh">
                  <section className="flex flex-col bg-muted/10 sm:p-5 pb-8 min-w-0 min-h-140">
                    <EvaluationHeader
                      examId={examId}
                      ...
                      evaluateDisabled={!selectedId || busy || evaluate.isPending}
                      submitPending={submit.isPending}
                      submitDisabled={!selectedId || !evaluation || submit.isPending}
                ...
                <ResizablePanel
                  ...
                  minSize="15%"
                  maxSize="50%"
                  className="min-w-0 h-dvh"
                >
                  <ReferencePanel key={selectedId ?? "no-entry"} capture={capture.data} />
                </ResizablePanel>
              </ResizablePanelGroup>
            </div>
          </SidebarInset>
