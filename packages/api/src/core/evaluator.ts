...
function scaledPoint(value: number[] | null, scale: number): number[] | null {
  return (
    value?.map((coordinate) => Math.max(0, Math.round(coordinate * scale))) ??
    null
  )
}
  ...
  return {
    ...
    annotations: evaluation.annotations.map((annotation) => ({
      ...
      x:
        annotation.x === null
          ? null
          : restoreCoordinate(annotation.x, size.width),
      y:
        annotation.y === null
          ? null
          : restoreCoordinate(annotation.y, size.height),
      ...
      commentAt: scaledPoint(annotation.commentAt, scale),
      start: scaledPoint(annotation.start, scale),
      end: scaledPoint(annotation.end, scale),
      points:
        annotation.points?.map((value) => scaledPoint(value, scale) ?? []) ??
        null,
    })),
  }
}
...
function usageOf(value: unknown): TokenUsage {
  ...
  const inputDetails = (usage.inputTokenDetails ?? {}) as Record<
    string,
    unknown
  >
...
export class CategoryEvaluator {
    ...
    return {
      ...
      questionScores: output.questionScores.map((questionScore) => ({
        ...
        score: Math.min(
          ...
          Math.max(0, questionScore.score)
        ),
  ...
  async evaluateFirst(input: {
    ...
    const [question, sampleAnswer, studentScript, originalSize] =
      await Promise.all([
    ...
    const canonicalSize = canonicalImageSize(
      originalSize.width,
      originalSize.height
    )
    const canonicalStudentScript = await sharp(studentScript)
      .resize(canonicalSize.width, canonicalSize.height, {
        fit: "fill",
    ...
    return restoreOriginalCoordinates(
      evaluation,
      1 / canonicalSize.scale,
      originalSize
    )
  }
