...
import type { GeneratedEvaluation, ScriptCandidate, ScriptCategory } from "@repo/api";
...
export async function getEntries(examId: string): Promise<Entry[]> {
  return (await parseResponse(client.api.categories[":examId"].entries.$get({ param: { examId } })))
    .entries;
}
