...
export const authDirectory =
  process.env.TUTOMATION_AUTH_DIR ?? `${repoRoot}/.auth`
...
function isCredentials(value: unknown): value is TeacherCredentials {
...
async function loadCredentials(): Promise<TeacherCredentials | null> {
...
export async function ensureTeacherAuthenticated(
  ...
  if (saved) {
    console.log(
      "Saved teacher credentials were rejected; please enter them again."
    )
  }
  ...
  await loginWithCredentials(page, await promptCredentials(terminal), context)
}
