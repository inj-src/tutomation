    ...
    .map((element): OverlayElement | null => {
      ...
      if (element.type === "line") {
        ...
        return {
          ...
          points,
          width:
            Math.max(1, ...points.map(([x]) => x)) -
            Math.min(0, ...points.map(([x]) => x)),
          height:
            Math.max(1, ...points.map(([, y]) => y)) -
            Math.min(0, ...points.map(([, y]) => y)),
          ...commonStyle({
            strokeWidth:
            ...
            roughness:
              typeof element.roughness === "number" ? element.roughness : 1.5,
          }),
        }
      }
...
window.__tutomationRenderExcalidraw = async (input) => {
    ...
    switch (element.type) {
      case "text": {
        ...
        if (typeof element.angle === "number") {
          const estimatedTextWidth =
            element.text.length * element.fontSize * 0.58
      ...
      case "line": {
        const points = element.points?.map(
          ([x, y]: [number, number]) =>
            [element.x + x, element.y + y] as [number, number]
        )
  ...
  const pngDataUrl = await new Promise<string>((resolve, reject) => {
    ...
    image.onerror = () =>
      reject(new Error("Could not rasterize the Excalidraw overlay."))
